...
class GoogleMapAPI {
     ...
     */
    function GoogleMapAPI($language = 'en', $map_id = 'map', $app_id = 'MyMapApp') {
        $this->map_id = $map_id;
        $this->sidebar_id = 'sidebar_' . $map_id;
        $this->api_key = GOOGLE_SERVER_KEY;
 ...
 function addWaypoints($lat, $lon, $stopover = TRUE)
 {
  ...
  function addWaypointByAddress($address,$stopover = TRUE)
  {
	...
	*/
    ...
    /**
     * get geocode address points for given coordinates  from Google
     *
     * @param string $lat
     * @param string $lon
     * @return bool|array false if can't be geocoded, array or geocdoes if successful
     */
    function geoGetAddress($lat,$lon) {
        $_url = "https://maps.googleapis.com/maps/api/geocode/json?latlng={$lat},{$lon}&key={$this->api_key}&language={$this->language}&result_type=locality";
        $_result = false;
        if($_result = $this->fetchURL($_url)) {
            $_result_parts = json_decode($_result);
            if($_result_parts->status!="OK"){
    ...
    /**
     * get place info By ID  from Google
     *
     * @param string $place_id
     * @return bool|array false if can't be geocoded, array or geocdoes if successful
     */
    function geoPlaceInfo($place_id) {
        $_url = "https://maps.googleapis.com/maps/api/place/details/json?placeid={$place_id}&key={$this->api_key}&language={$this->language}";
        $_result = false;
        if($_result = $this->fetchURL($_url)) {
            $_result_parts = json_decode($_result);
            if($_result_parts->status!="OK"){
            ...
            $_result = isset($_result_parts->result->address_components[2]) ? $_result_parts->result->address_components[2] : $_result_parts->result->address_components;
            return $_result_parts;
        }
    }

    /**
     * get place coords By ID  from Google
     *
     * @param string $place_id
     * @return bool|array false if can't be geocoded, array or geocdoes if successful
     */
    function geoPlaceCoords($place_id) {
        $_result_parts = $this->geoPlaceInfo($place_id);
        if(!$_result_parts) return false;
        $_result = $_result_parts->result->geometry->location;
        return $_result;
    }
}
